...
def get_order_distribution(
    ...
    注意：此接口只返回城市级别的统计信息，不包含详细地址等隐私信息。
    地址信息仅用于统计展示，不存储用户详细地址。
    ...
    Returns:
        订单城市分布统计数据，包含：
        - summary: 汇总信息（总订单数、总城市数等）
        - city_data: 城市列表（城市名、国家、省份、订单数、GMV等）
        - heatmap_data: 热力图数据格式（适合ECharts热力图）
    """
    from app.services.unified_statistics import UnifiedStatisticsService
    ...
    results = db.query(
        ...
        and_(*base_filters),
        Order.shipping_city.isnot(None),  # 必须有城市信息
        Order.shipping_city != ''  # 城市名不为空
    ).group_by(
        Order.shipping_city,
        Order.shipping_province,
        Order.shipping_country
    ).order_by(
        func.count(func.distinct(parent_order_key)).desc()  # 按订单数降序
    ).all()
    ...
    for row in results:
        ...
        city_data.append({
            "city": row.shipping_city or "未知",
            "province": row.shipping_province or "未知",
            "country": row.shipping_country or "未知",
    ...
    # 生成热力图数据格式（适合ECharts热力图）
    # 格式：[城市名, 订单数] 或 [城市名, 订单数, GMV]
    heatmap_data = [
        [
            row["city"],
            row["order_count"],
